...
export default function MarkAttendance() {
  ...
  const handleChange = (e) => {
    ...
    setLeaveInfo((prevState) => { return { ...prevState, [name]: value } })
  }
  ...
  const fetchData = async () => {
    try {
      ...
      if (response?.data[0]?.status == 'Late' || response?.data[0]?.status == 'Leave' || response?.data[0]?.status == 'Present') {
  ...
  const handleLeave = async (att_status) => {
    ...
    if (leaveInfo.leave_date && leaveInfo.reason) {
      if (leaveInfo.leave_date > todayDate) {
        ...
        try {
          const response = await axios.put(BaseUrl + '/leaverequest', { ...leaveInfo, emp_id });
  ...
  return (
    <div className="full-content" id="full-content">
      ...
      <div id="content">
        ...
        {attendanceMarked ? (
          <>
            ...
            <Button variant="primary" className='mt-3' onClick={() => { setShowLeaveModal(true) }}>
              Apply for Leave
            </Button>
          </>
        ) : (
      ...
      <Modal show={showLeaveModal} onHide={() => setShowLeaveModal(false)} className="custom-modal">
        <Modal.Header closeButton>
